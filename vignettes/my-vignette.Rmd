...
vignette: >
  %\VignetteIndexEntry{Spas tutorial}
...
```{r}
accuracy_sim <- function(n, m, times=100) {
  diffs <- numeric(0)
  for(i in 1:times) {
...
accuracy_results <- data.frame("n_obs" = c(500, 5000, 50000, 500000, 50000, 50000, 50000, 50000),
                               "n_feats" = rep(c(10, 10, 10, 10, 15, 20, 50, 100)))
...
diff.mat<-cbind(c(500,5000,50000,500000,50000,50000,50000,50000),c(10,10,10,10,15,20,50,100),rep(NA,8),rep(NA,8))
...
times_50000_10 <- run.feature.sim.linear(50000,10)
times_500000_10 <- run.feature.sim.linear(500000,10)
...
test1<-run.feature.sim(n=50000,m=10)
test2<-run.feature.sim(n=50000,m=15)
test3<-run.feature.sim(n=50000,m=20)
test4<-run.feature.sim(n=50000,m=50)
...
test.obs3<-run.feature.sim(n=50000,m=10)
test.obs4<-run.feature.sim(n=500000,m=10)
...
test.obs3.2<-test.obs3 %>% as.data.frame() %>% mutate(obs='50000 Observations')
test.obs4.2<-test.obs4 %>% as.data.frame() %>% mutate(obs='500000 Observations')

vary.obs<-rbind(test.obs1.2,test.obs2.2,test.obs3.2,test.obs4.2)
vary.obs$logtime=log(vary.obs$time)
vary.obs$obs = factor(vary.obs$obs, levels=c("500 Observations","5000 Observations","50000 Observations","500000 Observations"))
